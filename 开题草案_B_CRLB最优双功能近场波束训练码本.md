...
>
> ✅ **2026-08-12 判决：防线守得住（高置信，基于摘要+5 处独立来源核实；全文因 IEEE 付费墙未读，无 arXiv 版）。** 依据：该 letter 是 5 页短文，描述为"BS **同时做下行通信 + 目标感知**、用 **TTD+移相器混合预编码器**抑制 beam squint、min CRB s.t. **通信速率**、penalty+BCD"——优化变量是**数据阶段发射预编码器矩阵**，被感知的是**目标**（其角/距在感知接收端被估计），**通篇无 beam training / codebook / 初始接入 / 导频扫描 / 波束对齐**。整个近场宽带 ISAC-CRB 论文簇（DPP-ISAC / Fan Liu / 2311.05372 / 2302.01153 / 2412.13532 / 2603.27726）**全是预编码器或界分析，无一做"训练码本 / 初始接入主动实验设计"**。→ B 的收窄防线在这条线上**独占**。**残留动作**：仍建议拿到该 letter 全文 100% 确认没有隐藏的"初始接入"小节。（这一预警与判决采纳自用户 ChatGPT 讨论里的提醒，详见第九节末的分析。）
